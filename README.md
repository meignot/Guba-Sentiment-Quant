...
5. **板块龙头筛选** (`版块龙头票/select_sector_leader.py`)
   - 根据流通市值、利润规模（近3年无亏损）、成交额与技术趋势多维打分，自动挑选并分析板块内的领军龙头。
6. **龙虎榜席位分析** (`版块龙头票/lhb_analyser.py`)
   - 抓取个股龙虎榜上榜记录，分类统计机构专用及陆股通专用（外资）席位累计买入/卖出资金及净流向，透视主力博弈动向。
...
├── 版块龙头票/                    # 板块龙头股分析与龙虎榜追踪
│   ├── select_sector_leader.py   # 板块龙头股多维打分筛选程序
│   └── lhb_analyser.py           # 龙虎榜席位资金深度追踪分析器
...
#### **步骤 6：运行板块龙头筛选与龙虎榜席位追踪（新增）**
- 挑选“元件”等板块在流通市值、近期成交量和3年财务盈利维度上的龙头股票：
  ```bash
  python "版块龙头票/select_sector_leader.py"
  ```
  运行后结果将输出至控制台，并生成 `版块龙头票/元件版块龙头分析报告.md`。
  
- 追踪特定个股（如中京电子 002579）最新一期龙虎榜明细及机构与外资席位的买卖资金：
  ```bash
  python "版块龙头票/lhb_analyser.py" 002579
  ```
  或使用股票简称：
  ```bash
  python "版块龙头票/lhb_analyser.py" 胜宏科技
  ```
  分析结果将输出在终端，并自动保存席位明细报告至 `版块龙头票/` 目录下。
